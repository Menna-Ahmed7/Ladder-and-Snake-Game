...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	if (cellPos.VCell() == -1)
		...
		y = cellPos.VCell() * UI.CellHeight + UI.ToolBarHeight;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\Copy-Card.jpg";
	MenuItemImages[ITM_CUT_CARD] = "images\\Cut-Card.jpg";
	MenuItemImages[ITM_PASTE_CARD] = "images\\Paste-Card.jpg";
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\Delete-Object.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	///TODO: Prepare images for each menu item and add it to the list
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Input-Dice-Value.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\New-Game.jpg";
	MenuItemImages[ITM_PEXIT] = "images\\Exit.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if ((fromCell.HCell() == toCell.HCell()) && (toCell.VCell() < fromCell.VCell()) && fromCell.VCell() != 0)
	{
		...
		int width1 = UI.LadderlineWidth;
		pWind->SetPen(UI.LadderColor, width1);
		...
		int width2 = UI.LadderlineWidth;
		pWind->SetPen(UI.LadderColor, width2);
		...
		int liney = y2 + (UI.CellHeight - UI.LadderYOffset);
		pWind->DrawLine(x12, liney, x34, liney);
		liney += UI.CellHeight;
		while (liney < y1)
		{
			pWind->DrawLine(x12, liney, x34, liney);
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if ((fromCell.HCell() == toCell.HCell()) && (toCell.VCell() > fromCell.VCell()) && fromCell.VCell() != (NumHorizontalCells - 1))
	{
		...
		int width = UI.SnakelineWidth;
		pWind->SetPen(UI.SnakeColor, width);
		...
		int py4 = y1 + yChange;
		int px1 = cellStartX;
		int px2 = cellStartX + xChange / 2 + width;
		int px3 = cellStartX + 2 * (xChange / 2 + width);
		int px4 = cellStartX + xChange / 2 + width;
		int PX[4] = { px1,px2,px3,px4 };
		int PY[4] = { py1,py2,py3,py4 };
