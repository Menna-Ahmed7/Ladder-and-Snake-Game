...
public:
	...
	//TODO
	void SaveAll(ofstream& OutFile, Type GOBJ);
	int count(Type GOBJ);
	Player* getnextplayerincell(Player* p);            //to get the next player in grid
	bool IsOverLapping(GameObject* newobj) const;        
	void ClearAll();                                 //to clear the grid from any game object
	void setcurrentplayernumber(int num);           //to set the player number to play  
	void restartcards();                           //to restart the cards
	Card* GetCardofCell(const CellPosition& c);
	Player* GetPoorestPlayer();                  //specific for card 12        //returns the poorest player in the game
	bool hasgameobject(const CellPosition& pos);
